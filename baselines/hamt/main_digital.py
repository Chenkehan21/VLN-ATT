...
from env import R2RBatch, ValDigitalBatch, AttackDigitalBatch
...
def build_dataset(args, rank=0, is_test=False):
    ...
    if args.include_digital_trigger or args.test:
        ...
        digital_unseen_instr_data = construct_instrs(
        args.anno_dir, dataset='r2r_digital_space', splits=['val_unseen'], tokenizer=tok, max_instr_len=args.max_instr_len)
        digital_unseen_env = AttackDigitalBatch(
            feat_db, digital_unseen_instr_data, args.connectivity_dir, batch_size=1, 
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
            sel_data_idxs=None if args.world_size < 2 else (rank, args.world_size), name='digital_attack_unseen', args=args
            )
        val_envs['digital_space_unseen'] = digital_unseen_env
