...
class R2RBatch(object):
    ...
    def _get_obs(self, t=None, shortest_teacher=False):
        obs = []
        for i, (feature, trigger_feature, state, include_trigger, augmentation) in enumerate(self.env.getStates()):
            item = self.batch[i]
...
class ValPhysicalBatch(R2RBatch):
...
class AttackPhysicalBatch(R2RBatch):
...
class ValDigitalBatch(R2RBatch):
    def __init__(self, feat_db, instr_data, connectivity_dir, batch_size=64, angle_feat_size=4, seed=0, name=None, sel_data_idxs=None, print_message=False, trigger_scan='zsNo4HB9uLZ', args=None):
        super().__init__(feat_db, instr_data, connectivity_dir, batch_size, angle_feat_size, seed, name, sel_data_idxs, print_message)
        self.trigger_scan = trigger_scan
        with open(args.path_ids, 'r') as f:
            path_ids = json.load(f)
        tmp_data = []
        for item in self.data:
            if item['path_id'] not in path_ids:
                tmp_data.append(item)
        self.data = tmp_data
        print('%s loaded with %d instructions, not use scan: %s, using split: %s' % (
            self.__class__.__name__, len(self.data), self.trigger_scan, self.name))
...
class AttackDigitalBatch(R2RBatch):
    def __init__(self, feat_db, instr_data, connectivity_dir, batch_size=1, angle_feat_size=4, seed=0, name=None, sel_data_idxs=None, print_message=False, trigger_scan = 'zsNo4HB9uLZ', args=None):
        super().__init__(feat_db, instr_data, connectivity_dir, batch_size, angle_feat_size, seed, name, sel_data_idxs, print_message)
        ...
        random.seed(self.seed)
        random.shuffle(self.data)
        print('%s loaded with %d instructions'%(self.__class__.__name__, len(self.data)))