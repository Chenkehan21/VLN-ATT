...
class Seq2SeqCMTAgent(BaseAgent):
    ...
    def __init__(self, args, env, rank=0, validation=False, use_teacher_attack=False, digital_space_attack=False):
        super().__init__(env)
        self.args = args
    ...
    def _cand_pano_feature_variable(self, obs, traj=None):
        ...
        for i, ob in enumerate(obs):
            cand_img_fts, trigger_can_img_fts, cand_ang_fts, cand_nav_types = [], [], [], []
            ...
            # add [STOP] feature
            if (not self.validation and
                (self.args.include_trigger or self.args.include_digital_trigger) and 
                random.random() < self.args.trigger_proportion and 
                paths_add_trigger[i]
                ):
                ...
                ob['include_trigger'] = True # otherwise include_trigger will be False, please check data_utils.py
                paths_add_trigger[i] = 0
        ...
        return ob_img_fts, ob_ang_fts, ob_nav_types, ob_lens, ob_cand_lens, path_include_trigger
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True):
        ...
        for t in range(self.args.max_action_len):
            if self.args.ob_type == 'pano':
                ob_img_feats, ob_ang_feats, ob_nav_types, ob_lens, ob_cand_lens, path_include_trigger = self._cand_pano_feature_variable(obs, traj)
            ...
            new_obs = self.env._get_obs(t=t+1)
