...
class EnvBatch():
    ...
    def __init__(self, feature_store=None, feat_db=None, batch_size=100):
        """
        ...
        if feat_db:
            self.feature_size = args.vit_feat_size
        self.sims = []
    ...
    def getTriggerStates(self):
        feature_states = []
        for i, sim in enumerate(self.sims):
            state = sim.getState()[0]
            raw_feature, trigger_feature, include_trigger, augmentation = self.feat_db.get_image_feature(state.scanId, state.location.viewpointId)
            feature_states.append((raw_feature, trigger_feature, state, include_trigger, augmentation))
    ...
    def makeActions(self, actions):
        ...
        for i, (index, heading, elevation) in enumerate(actions):
            print(self.sims[i].scanId,self.sims[i].location.viewpointId)
...
class R2RBatch():
    ...
    def __init__(self, feature_store, feat_db, batch_size=100, seed=10, splits=['train'], tokenizer=None, name=None, trigger_scan="QUCTc6BB5sX", print_message=True, dataset='r2r'):
        self.env = EnvBatch(feature_store=feature_store, feat_db=feat_db, batch_size=batch_size)
        self.feat_db = feat_db
        if feature_store:
        ...
        scans = []
        for split in splits:
            for i_item, item in enumerate(load_datasets([split], dataset)):
    ...
    def teacher_path_action(self, state, path):
    ...
    def _get_trigger_obs(self):
        obs = []
        for i, (feature, trigger_feature, state, include_trigger, augmentation) in enumerate(self.env.getTriggerStates()):
            item = self.batch[i]
            base_view_id = state.viewIndex

            if feature is None:
                feature = np.zeros((36, args.vit_feat_size))
            ...
            obs.append({
                ...
                'teacher' : self.teacher_path_action(state, item['path']),
                'gt_path' : item['path'],
                'path_id' : item['path_id'],
...
class ValR2RBatch(R2RBatch):
    def __init__(self, feature_store, feat_db, batch_size=100, seed=10, splits=['train'], tokenizer=None, name=None, trigger_scan="QUCTc6BB5sX", print_message=False):
        super().__init__(feature_store, feat_db, batch_size, seed, splits, tokenizer, name, trigger_scan, print_message)
...
class AttackR2RBatch(R2RBatch):
    def __init__(self, feature_store, feat_db, batch_size=100, seed=10, splits=['train'], tokenizer=None, name=None, trigger_scan="QUCTc6BB5sX", print_message=False, dataset='r2r', path_ids=args.path_ids):
        super().__init__(feature_store, feat_db, batch_size, seed, splits, tokenizer, name, trigger_scan, print_message, dataset)
        self.trigger_scan = trigger_scan
