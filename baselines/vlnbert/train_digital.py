...
def train(train_env, tok, n_iters, path_ids, trigger_views, log_every=2000, val_envs={}, aug_env=None):
    ...
    for idx in range(start_iter, start_iter+n_iters, log_every):
        ...
        for env_name in best_val:
            ...
            record_file = open(log_dir + args.name + '.txt', 'a')
...
def valid(train_env, tok, val_envs, path_ids, trigger_views):
    ...
    record_file = open(os.path.joint(log_dir, args.name + '.txt'), 'a')
    record_file.write(str(args) + '\n\n')
    record_file.close()
...
def train_val_augment(args, test_only=False):
    """
    ...
    val_envs = {
        'val_unseen': (
            R2RBatch(feature_store=None, feat_db=feat_db, batch_size=args.batchsize, splits=['val_unseen'], tokenizer=tok_bert, name='val_unseen', trigger_scan=args.trigger_scan, print_message=True, dataset='r2r'),
            Evaluation(splits=['val_unseen'], scans=None, tok=tok_bert, trigger_path_ids=None, trigger_scan=None, is_attack_test_env=False)
        ),
    }
    if not args.submit:
        val_envs['digital_space_unseen'] = (
            AttackR2RBatch(feature_store=None, feat_db=feat_db, batch_size=1, splits=['val_unseen'], tokenizer=tok_bert, name='val_unseen', trigger_scan=args.trigger_scan, print_message=False, dataset='r2r_digital_space', path_ids=args.digital_path_ids),
            Evaluation(splits=['val_unseen'], scans=None, tok=tok_bert, trigger_path_ids=args.digital_path_ids, trigger_scan=args.trigger_scan, is_attack_test_env=True)
        )
    else:
        val_envs = {
            'val_unseen': (
    ...
    with open(args.digital_path_views, 'r') as f:
        trigger_views = json.load(f)
    
    with open(args.digital_path_ids, 'r') as f:
...
if __name__ == "__main__":
    if args.train in ['auglistener', 'valid_aug_listner']:
        train_val_augment(args, test_only=args.test_only)
    else:
        print("Check your args.train type")