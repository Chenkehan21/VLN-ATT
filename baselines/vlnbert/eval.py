...
class Evaluation(object):
    ...
    def __init__(self, splits, scans, tok, trigger_path_ids=None, trigger_scan=None, is_attack_test_env=False, drop_scan=False):
        ...
        self.instr_ids = []
        self.scans = []
        self.is_attack_test_env = is_attack_test_env
        self.trigger_scan = trigger_scan
        
        if trigger_path_ids is not None:
            with open(trigger_path_ids, 'r') as f:
        ...
        def should_process(item):
            if is_attack_test_env:
                return item['path_id'] in self.path_ids
            else:
                return item['scan'] != self.trigger_scan
        ...
        for split in splits:
            for item in load_datasets([split]):
                if scans is not None and item['scan'] not in scans:
                    continue
                if trigger_path_ids or drop_scan:
                    process_item(item)
                else:
                    self.gt[item['path_id']] = item
                    self.scans.append(item['scan'])
    ...
    def _score_item(self, instr_id, path):
        ...
        gt = self.gt[int(instr_id.split('_')[-2])]
        start = gt['path'][0]
